...
void
paillier_para_enc(int n, paillier_pubkey_t* pub, paillier_plaintext_t SS[], paillier_ciphertext_t S[])
{
    int i=0;
...
void
paillier_para_dec(int n, paillier_pubkey_t* pub, paillier_prvkey_t* prv, paillier_ciphertext_t S[], paillier_plaintext_t SS[])
{
    int i=0;
    #pragma omp parallel for
    for(i=0; i<n; i++)
    {
        SS[i] = *(paillier_dec(NULL, pub, prv, &(S[i])));
    }
}

void
paillier_cipheradd( int n, int L, paillier_pubkey_t* pub, paillier_ciphertext_t et[], 
                   paillier_ciphertext_t S[])
{
    int i=0, j=0;
    for ( j=0; j<n; j+=8){
